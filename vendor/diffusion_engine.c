...
#define _POSIX_C_SOURCE 200809L  /* fmemopen — RGF mem-loader (load WGHT from memory, not a path) */
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
#include <math.h>
...
/* Load weights from an open stream (file or fmemopen'd blob). Caller closes f. */
static DiffWeights* load_weights_f(FILE* f) {
    uint32_t magic;
    int32_t n;
    if (fread(&magic, 4, 1, f) != 1 || fread(&n, 4, 1, f) != 1) return NULL;
    if (magic != 0x4E544F52) { fprintf(stderr, "Bad magic: 0x%08X\n", magic); return NULL; }
    if (n != D_N_TENSORS) { fprintf(stderr, "Expected %d tensors, got %d\n", D_N_TENSORS, n); return NULL; }
...
static DiffWeights* load_weights(const char* path) {
    ...
    DiffWeights* w = load_weights_f(f);
    fclose(f);
    return w;
}
...
/* RGF: load weights from an in-memory blob (the WGHT chunk) — no filesystem
 * path. fmemopen wraps the blob as a read-only stream; fread stays bounded by
 * the blob size, and load_mat/load_vec's ndim guards cover the rest. This is
 * how the .rgf viewer loads its model from memory, not from a path. */
int diff_load_mem(const unsigned char* blob, size_t n) {
    if (g_weights) { free_weights(g_weights); g_weights = NULL; }
    FILE* f = fmemopen((void*)blob, n, "rb");
    if (!f) return -1;
    g_weights = load_weights_f(f);
    fclose(f);
    if (!g_scratch) g_scratch = (Scratch*)calloc(1, sizeof(Scratch));
    return g_weights ? 0 : -1;
}
